...
- [Windows] and [Linux] In a command prompt type: `obabel`\
  ...
  - [Windows] and [Linux] From a Python prompt verify that the following statement `from openbabel import pybel` executes without errors. If an error is issued, check the compatibility of the openbabel-wheel version installed under step (3) with your version of Python (on Windows, [openbabel-wheel compatibility](https://pypi.org/project/openbabel-wheel/) may lag behind the latest stable Python release version).
...
If all you wish to do is to run the S2AS program for your own system of components, this is a relatively straightforward task. The following inputs (files) need to be provided.
...
3. Copy this file into the `InputFiles` folder. Do not list the SMILES of water (or other inorganic components) in that file; S2AS v1.0 only supports the parsing of organic molecules (see details in the article by Amaladhasan et al., 2026).
